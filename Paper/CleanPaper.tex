...
%This is then combined with the cell's EMT score and a threshold value determines if the cell subsequently undergoes EMT or MET.
%In doing this, we are assuming that the epithelial and mesenchymal phenotypes are stable and cells in a given type tend to move towards that 
%This represents cell-extrinsic sources shaping the trajectory of the cell along the EMT spectrum.
%Intrinsically, we assume that a cell is likely to keep its same phenotype, so we mesenchymal cells are more li
%If the difference is large enough, the cell undergoes EMT, otherwise it undergoes MET.
%This value $\tau_i$ is given by the following equation:
%EMT depends on TGF-$\beta$ concentration through a Hill-type function that determines the likelihood of EMT (or the reverse MET) through its magnitude relative to $\tau_{i}$. This is described by the equation:
    \newline
...
% lead to more rapid invasion or if it survives to a certain time point, a longer time to invasion.
