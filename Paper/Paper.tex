...
Recent work has shown that -- rather than being a binary process -- at least two stable intermediate states exist on the EMT axis \cite{hong2015ovol2, jolly15_coupling}. Ongoing investigations into the plasticity and stability of EMT overlap with studies of discrete vs. continuous processes elsewhere, such as in cell differentiation \cite{moris16_transition}. Moreover, intermediate states have emerged as a central mechanisms by which cell fates (and the noise inherent within in them) can be controlled \cite{maclean18_exploring, ta16_controlling, rackauckas18_meanindependent}  




While EMT has for some time been assumed to impact metastasis, more recently EMT-related phenotypes have been linked to other aspects of both tumor initiation and progression \cite{nieto2016emt}. In addition, TGF-$\beta$ is a master regulator of EMT, thus linking it closely with the signaling networks involved in immune responses [REF].
