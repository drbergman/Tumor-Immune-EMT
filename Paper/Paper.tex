...
Year-by-year, new breakthroughs in the field produce new, effective drugs and therapies that have a significant impact on patient health and survival\cite{pardoll2012blockade}\cite{restifo2012adoptive}.  % More examples? Reviews or non-reviews? Explain some of the examples?
...
Upon successful clearance, they themselves are then deactivated and removed from the immune population. % Should this be justified here or mentioned in the intro?
...
$$ p_{\text{NK}} =  \delta_M \frac{\text{NK}}{N/N_{1}+\text{NK}} E_{\text{NK}} \frac{1}{1+\text{Treg}/\text{Treg}_{\text{EC50}}} \cdot $$
$$ \qquad(1-\delta_{IEM}\text{IE}_{\text{up}})(1-\delta_{MES}m_{\text{IE}})$$
...
\paragraph{Rest/$G_0$} %use both for this section heading?
...
$$ p_{G0} = 1 + \delta_{\text{MES}}p(1+\delta_{\text{PM}}\text{prolif}_{\text{up}})m_{GA}\frac{N_0}{N_0+N} $$ %p_{G_0},p_{rest},p_{\text{rest}},p_{GA}???

The reason for adding that term is due to the understanding that mesenchymal cells are not proliferating as fast because they are instead remaining in the $G_0$ phase. % is this the right way to word this?
...
This represents the fact that individual immune cells lose efficacy as they carry out their effector functions. % Similar to previous comment: should I cite reference for this here or put this up in the intro?
...
If the cell does not undergo a mutation, then its probability of mutating during subsequent cell increases. % Updated this after Adam had question. Does this make sense as is?
...
C. Example survival curve for one cohort of patients with basal parameter values. % I should list out all parameter values?
