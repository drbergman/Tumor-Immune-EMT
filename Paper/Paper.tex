...
Year-by-year, more breakthroughs in the field produce effective new drugs and therapies that are having a significant impact on patient health and survival\cite{restifo2012adoptive}.  [examples] 
...
EMT has an important yet complicated role in tumor initiation, progression, and metastasis\cite{nieto2016emt}. % References more specific to this claim might be in this paper's references. 
As mesenchymal cells often exhibit stem-like properties, their existence has been tied to the initiation and progression of cancer\cite{nieto2016emt}. % References more specific to this claim might be in this paper's references.
...
The immune system affects EMT by releasing a flurry of cytokines upon arrival in the TME \cite{terry2017new}. 
In particular, regulatory T cells (Tregs) release copious quantities of the EMT-driving transforming growth factor beta (TGF-$\beta$)\cite{terry2017new}. 
...
The second is that mesenchymal cells proliferate at a slower rate. Ref % Adam said he had a reference in mind for this.
...
Once cytotoxic immune cells, for example NKs and CTLs, have targeted a cell to lyse, they necessarily must make a physical connection with that cell, termed an immunological synapse\cite{terry2017new}. 
However, there is preliminary evidence that due to down-regulated surface markers on mesenchymal cells among other structural changes, they are refractory to this immunological task thus rendering them less susceptible to immune clearance\cite{terry2017new}. % 
...
C. Example survival curve for one cohort of patients with parameters XXXXX. % I should list out all parameter values?
...
Notice that at cell cycle number 541 the proportion of mutated cells reaches 50\% and so the patient has a Time to Cancer of 541, which translates to 405.75 days. % translate to days like this?
...
Meanwhile, all the cells become mesenchymal for a short duration before most transition back to an epithelial state.% Once the Time to Cancer has been determined for a patient, most simulations shown later stop. [??? mean to say: we stop the simulation?]
