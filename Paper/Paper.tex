...
\begin{figure}
\center
{\includegraphics[width=0.98\textwidth]{Figure1/Figure1.pdf}}
...
\par
The role that TGF-$\beta$ plays throughout the tumor and its microenvironment also requires further research. We found that, below a threshold, reduction of TGF-$\beta$ increases the Time to Cancer  (Fig. \ref{fig:FirstSurvivalCurves}E), thus reducing expression levels of TGF-$\beta$ in the tumor microenvironment benefits survival. However, TGF-$\beta$ is a master regulator implicated in numerous cellular signaling processes, and changing the concentrations of TGF-$\beta$ even in a local tumor microenvironment could have large off-target effects. Thus it might be more fruitful to target signaling factors downstream of TGF-$\beta$ that still have the ability to modulate epithelial cell dynamics: this is an area of active and future investigation, we are currently developing a larger TGF-$\beta$ signaling pathway module with appropriate crosstalks to  epithelial/mesenchymal/immune cell functions to be incorporated into the model.
\par
